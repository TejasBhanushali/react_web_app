...
export  default class ColorV extends React.Component {
  ...
  publishData(data){
      console.log("************  publishData() method has been called  ****************"+data);
      PubSub.publish('products', data);
    }
  
  render() {
	{/* Check uiElement is List or what  */}   
	//console.log("************ Inside ColorV render() ****************");
    ...
    var  attrVal = 
        productV.productVariantDetailsSet.filter((obj,index) =>
        (function() {
              ...
              <ul className="eaKBCI">
                 {this.props.product.variantMap[this.props.variantKey].map((obj,index) =>
                   ...
                   <li className="_3XkO0t" id={this.props.variantKey +"-"+ obj.attributeValue + "-" + index } key={index} >
                       <a className={"_2aRfTu" + 
                       (attrVal == obj.attributeValue ? ' _26Dt15' : 
                           this.props.product.productInOthVariants[this.props.variantKey].includes(obj.attributeValue) ?
                            '':' _3-ViMg')} data-img={imgBasUrl + obj.attriuteImgUrl} href="" 
                            onClick={() => this.props.onClick(variantKey + ":" + obj.attributeValue)} 
                            id={"a"+ this.props.variantKey +"-"+ obj.attributeValue + "-" + index }>
                          <div className="_1lovqS _34r2i7" data-img={imgBasUrl + obj.attriuteImgUrl} >
                             <img src={imgBasUrl + obj.attriuteImgUrl} className="bKNGJM" /></div>
                       </a>

                    </li> )}
                 
                    <li className="WNcvgz" ></li>
                </ul>
            </div>
